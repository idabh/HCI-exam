...
def page1():
    ...
    with st.sidebar:
        ...
        st.expander
        education_level = st.select_slider("Minimum education level", options=["High School", "Bachelor", "Masters", "Ph.d.", "Postdoc"],  help="Choose minimum education level needed")
        ...
        sql = st.slider("SQL test score", min_value=0, max_value=50, value=0, step=1, format=None,  help="The performances of the candidates on the SQL test", on_change=None, args=None, kwargs=None,  disabled=False)
        
        grade = st.slider('Grade average', 0,50,0)
        english = st.slider('English proficiency',0,10,0)
    ...
    radar_data = pd.DataFrame(dict(
        ...
        grade,
        sql/5],
        theta=['python skills','education_level','english','grade', 'sql'],
